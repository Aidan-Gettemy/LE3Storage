...
function status = makeScatter(name,saveID,Predict,Actual,scores)
    ...
    xlabel("Actual",FontSize=20)
    ylabel("Estimated",FontSize=20)
    title("Tree Regression: "+name, 'FontSize',25);
    set(gca,'FontSize',16);
    set(gca,'LineWidth',2);
...
function status = makeQQ(name,saveID,Predict,Actual,scores)
    ...
    set(gca,'TickDir','out')
    set(gca,'FontSize',16)                    
    xlabel('Data Quantiles','FontSize',20)
    ylabel('Estimate Quantiles','FontSize',20)
    title("Tree Regression: "+name ,'FontSize',25)
